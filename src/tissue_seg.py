...
if __name__ == '__main__':
    ...
    assert os.path.isdir(args.input_dir), 'Input directory "{}" does not exist!'.format(args.input_dir)

    # Make output directory if not exists
    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)

    img_names = [pathname.split('/')[-1] for pathname in glob.glob(os.path.join(args.input_dir, "*.svs"))]
    img_names = sorted(img_names)

    stain_file_names = [pathname.split('/')[-1] for pathname in glob.glob(os.path.join(args.input_dir, 'stain_contrast', "*.txt"))]
    stain_file_names = sorted(stain_file_names)
    main_logger.info('Inputs: %d .svs WSIs with %d .txt stain_contrast files', len(img_names), len(stain_file_names))
    ...
    for i, img_name in enumerate(img_names):
        ...
        tissue_mask_arr, tissue_timer_result \
                = separate_tissue(img_arr, stain_file_path, save_tissue_mask=True, save_tissue_mask_dir=args.output_dir)
        timer_result.append(time())
